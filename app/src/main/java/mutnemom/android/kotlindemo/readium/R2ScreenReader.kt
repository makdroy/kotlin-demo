...
class R2ScreenReader(
    ...
    private var epubName: String, initialResourceIndex: Int
) {

    companion object {
        private val LOG_TAG = R2ScreenReader::class.java.simpleName
    }
    ...
    var currentUtterance: Int = 0
        ...
        set(value) {
            ...
            if (BuildConfig.DEBUG) Log.e(LOG_TAG, "Current utterance index: $currentUtterance")
        }
    ...
    init {
        ...
        textToSpeech = TextToSpeech(context,
            TextToSpeech.OnInitListener { status ->
                ...
                if (BuildConfig.DEBUG) Log.e(LOG_TAG, "textToSpeech initialization status: $initialized")
            })
    }
    ...
    private fun setTTSCallbacks(): Boolean {
        val res = textToSpeech.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
            ...
            override fun onError(utteranceId: String?) {
                if (BuildConfig.DEBUG) Log.e(LOG_TAG, "Error saying: ${utterances[utteranceId!!.toInt()]}")
            }
        })

        if (res == TextToSpeech.ERROR) {
            if (BuildConfig.DEBUG) Log.e(LOG_TAG, "TTS failed to set callbacks")
            return false
        }
    ...
    private fun startReading(): Boolean {
        ...
        if (initialized && configure()) {
            if (currentUtterance >= utterances.size) {
                if (BuildConfig.DEBUG) Log.e(LOG_TAG, "Invalid currentUtterance value: $currentUtterance . Expected less than $utterances.size")
                currentUtterance = 0
            }
    ...
    fun setSpeechSpeed(speed: Float, restart: Boolean): Boolean {
        try {
            ...
            if (BuildConfig.DEBUG) Log.e(LOG_TAG, e.toString())
            return false
        }
    ...
    private fun addToUtterancesQueue(utterance: String, index: Int): Boolean {
        if (textToSpeech.speak(utterance, TextToSpeech.QUEUE_ADD, null, index.toString()) == TextToSpeech.ERROR) {
            if (BuildConfig.DEBUG) Log.e(LOG_TAG, "Error while adding utterance: $utterance to the TTS queue")
            return false
        }
    ...
    private fun flushUtterancesQueue(): Boolean {
        if (textToSpeech.speak("", TextToSpeech.QUEUE_FLUSH, null, null) == TextToSpeech.ERROR) {
            if (BuildConfig.DEBUG) Log.e(LOG_TAG, "Error while flushing TTS queue.")
            return false
        }
